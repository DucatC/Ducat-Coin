...
namespace {
    class CDucatAddressVisitor : public boost::static_visitor<bool> {
    private:
        CDucatAddress *addr;
    public:
        CDucatAddressVisitor(CDucatAddress *addrIn) : addr(addrIn) { }
...
bool CDucatAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CDucatAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CDucatAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CDucatAddressVisitor(this), dest);
}

bool CDucatAddress::IsValid() const {
...
CTxDestination CDucatAddress::Get() const {
    if (!IsValid())
...
bool CDucatAddress::GetKeyID(CKeyID &keyID) const {
...
bool CDucatAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CDucatSecret::SetKey(const CKey& vchSecret) {
...
CKey CDucatSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CDucatSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CDucatSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CDucatSecret::SetString(const std::string& strSecret) {
