...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeDucatAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDucatAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeDucatAmount = strAnonymizeDucatAmount.remove(strAnonymizeDucatAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDucatAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeDucatAmount*COIN) nMaxToAnonymize = nAnonymizeDucatAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeDucatAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeDucatAmount));
        strAnonymizeDucatAmount = strAnonymizeDucatAmount.remove(strAnonymizeDucatAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDucatAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
