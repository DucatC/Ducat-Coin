...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDucatAmount"))
        settings.setValue("nAnonymizeDucatAmount", 1000);
    nAnonymizeDucatAmount = settings.value("nAnonymizeDucatAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeDucatAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeDucatAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeDucatAmount:
            return QVariant(nAnonymizeDucatAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeDucatAmount:
            nAnonymizeDucatAmount = value.toInt();
            settings.setValue("nAnonymizeDucatAmount", nAnonymizeDucatAmount);
            emit AnonymizeDucatAmountChanged(nAnonymizeDucatAmount);
            break;
        default:
            break;
        }
